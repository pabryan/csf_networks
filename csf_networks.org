...
We consider networks with a single loop. For us this means we have a piecewise smooth embedding for the loop:
\[
...
\]
and we always consider $i$ modulo $k$. Let us also write $\gamma_i = \gamma|_{[a_{i-1}, a_i]}$ for the smooth connected arcs, which by assumption are smooth up to the boundary with $\gamma_i(a_i) = \gamma_{i+1}(a_i)$ ($i$ modulo $k$).
...
\end{equation}
In other words, the tangents make equal angles at the nodes. See figure \ref{fg:network}.
