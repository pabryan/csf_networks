...
, embedded curves $(L_i: [0,a) \to \RR^2)_{i=1}^n$ and a piecewise smooth, closed, embedded curve $\gamma$, written $\gamma = (\gamma_i: [0,1] \to \RR^2)_{i=1}^n$ with each $\gamma_i$ a smooth embedding, such that
...
We will work with the loop.

#+BEGIN_defn
A /triple-compatible loop with $k$ nodes/ is a continuous embedding $\network : \sphere^1 \to \RR^2$ that is smooth on $\sphere^1 \setdiff \{a_i\}_{i=1}^k\}$ for $k$ distinct points, $a_i \in \sphere^1$ (the nodes) such that the /interior angles/ made by the one-sided tangent vectors at each $a_i$ is $2\pi/3$.
#+END_defn

The purpose of this paper is to study the evolution of triple-compatible loops under the curve shortening flow.
...
It will be convenient for us to fix a parametrisation of our smooth family of loops. Thus we assume,
\[
(x, t) \in [0, L] \times [0, T) \mapsto \network(x, t) \in \RR^2
\]
with a partition,
\[
0 = a_0 < a_1 < \cdots < a_{k-1} < a_k = L
\]
such that each $\gamma_i = \gamma|_{(a_{i-1}, a_i)}$ is smooth, $\gamma_{i+1}(a_i) = \gamma_i(a_i)$ (with $i$ taken modulo $k$) and writing $T^+(a_i) = \lim_{x\to a_i^+} \gamma_{i+1}'(x)$, $T^-(a_i) = \lim_{x\to a_i^-} \gamma_{i}'(x)$ we have
\[
\ip{T^+}{T^-} = 1/2.
\]
To say that the interior angle is $2\pi/3$ requires also that
\[
\ip{T^+}{N^-} = , \quad \ip{T^-}{N^+} =
\]
